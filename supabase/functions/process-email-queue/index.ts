  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      try {
        ...
        if (typeof payload.idempotency_key === 'string' && payload.idempotency_key.trim().length > 0) {
          emailPayload.idempotency_key = payload.idempotency_key
        } else if (typeof payload.message_id === 'string' && payload.message_id.trim().length > 0) {
          emailPayload.idempotency_key = payload.message_id
        }

        if (typeof payload.unsubscribe_token === 'string' && payload.unsubscribe_token.trim().length > 0) {
